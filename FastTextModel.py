...
import jieba
from data.data_processing import load_label_name_map,load_stop_word_list
...
def load_model_to_test():
    ...
    for w in range(1, 3):
        ...
        for i in range(5, 31):
            ...
            data_rows.append([marco_precision, marco_recall, marco_f1])
        ...
        plt.plot(names, all_marco_recall, label='marco-R')
        ...
        plt.show()
    write2csv(data_rows1, data_rows2)
...
import csv
def write2csv(data_rows1,data_rows2):
    with open("data_ft_1.csv", "w") as csvfile:
        writer = csv.writer(csvfile)
        # 先写入columns_name
        writer.writerow(['P', 'R','F1'])
        # 写入多行用writerows
        writer.writerows(data_rows1)
    with open("data_ft_2.csv", "w") as csvfile:
        writer = csv.writer(csvfile)
        # 先写入columns_name
        writer.writerow(['P', 'R','F1'])
        # 写入多行用writerows
        writer.writerows(data_rows2)
